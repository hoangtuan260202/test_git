...
def load_real_samples():
    ...
    # Chuyển đổi ảnh màu thành ảnh xám và thay đổi kích thước
    gray_images = []
    for image in trainX:
        # Chuyển đổi thành ảnh xám
        gray_image = Image.fromarray(image).convert('L')
        
        # Thay đổi kích thước từ 32x32 thành 28x28
        resized_gray_image = gray_image.resize((28, 28))
        
        # Chuyển đổi thành mảng numpy
        resized_gray_array = np.array(resized_gray_image)
        
        # Mở rộng kích thước (nếu cần thiết)
        resized_gray_array = expand_dims(resized_gray_array, axis=-1)
        
        # Thêm ảnh xám vào danh sách
        gray_images.append(resized_gray_array)
    ...
    # Scale from [0,255] to [-1,1]
    gray_images = (gray_images - 127.5) / 127.5
    
    return [gray_images, trainy]
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=20, n_batch=128):
    ...
    g_model.save('cgan_generator_cifar100.keras') 
